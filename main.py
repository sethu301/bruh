...
async def watch(ctx, *, channel_name=None):
    ...
    await ctx.send(f"📺t Now streaming: **{channel_name.upper()}**")
...
async def play(ctx, *, query):
    ...
    if not ctx.voice_client:
        try:
            ...
            await ctx.send("⚠️ Failed to connect to the voice channel (timeout). Try again.")
            return
    else:
        vc = ctx.voice_client
    ...
    try:
        ...
        await ctx.send(f"❌ Failed to get audio: `{str(e)}`")
        return
...
async def channels(ctx):
    fancy_channel_list = """
📺t **Available Malayalam TV Channels** 📺t
━━━━━━━━━━━━━━━━━━━━━━
🔹 asianet news
🔹 news 18 kerala
🔹 manorama news
